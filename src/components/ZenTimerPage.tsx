...
export function ZenTimerPage({ allQuotes }: ZenTimerPageProps) {
  ...
  const presetTimes = [60, 120, 180, 240, 300];
          ...
          <div className="relative w-64 h-64 mx-auto mb-6">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                ...
                <div className="text-sm text-muted-foreground">{isRunning ? t("timer.breathing") : t("timer.ready")}</div>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-center gap-4 mb-6">
            {!isRunning ? (
              <Button onClick={startTimer} variant="zen" size="lg" className="glow-primary">
                <MdPlayArrow className="w-6 h-6 mr-2" /> {t("timer.start")}
              </Button>
            ) : (
              <Button onClick={pauseTimer} variant="outline" size="lg">
                <MdPause className="w-6 h-6 mr-2" /> {t("timer.pause")}
              </Button>
            )}
            <Button onClick={stopTimer} variant="outline" size="lg">
              <MdStop className="w-6 h-6 mr-2" /> {t("timer.stop")}
            </Button>
          </div>

          <Button onClick={() => setVibrateEnabled(!vibrateEnabled)} variant="ghost" size="sm" className={vibrateEnabled ? 'text-primary' : 'text-muted-foreground'}>
            <MdVibration className="w-4 h-4 mr-2" /> {t("timer.vibrateOnFinish")}
          </Button>
        </div>
        ...
        <div className="glass-card p-6 mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
            {presetTimes.map(time => (
              <Button key={time} onClick={() => { setDuration(time); setTimeLeft(time); setIsRunning(false); }} variant={duration === time ? "zen" : "outline"} size="sm" className="transition-all duration-300 hover:scale-105">
                {time / 60}{t("timer.min")}
              </Button>
            ))}
          </div>
